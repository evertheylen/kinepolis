...
De frontend gebruikt npyscreen voor de UI. Elk scherm bevat normaal genoeg hints om je weg door de frontend te vinden.

## Backend

De backend is een volledig werkende python shell.
De dictionary ```data``` bevat alle informatie die wordt opgeslagen.
Voor meer info, zie het bestand backend.py zelf.

## Sorteringsalgoritmen

Alle sorteringsalgoritmen worden op dezelfde manier opgeroepen:

```python
# stel l is een lijst met users, en we willen sorteren op ID
sortFunc(l, 'ID')
# nu is l gesorteerd
```
...
+insert(el: ElementType) Bool
# insert het el. geeft succes van de operatie terug.
